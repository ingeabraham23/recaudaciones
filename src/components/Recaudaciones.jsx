// eslint-disable-next-line no-unused-vars
import React from "react";
import "./Recaudaciones.css";

const Recaudaciones = () => {
  return (
    <div className="contenedor">
      <h2>
        Política de Cooperación entre Compañeros operadores y checadores de
        urbanos rojos en Caso de Hospitalización o Fallecimiento de Familiares.
      </h2>
      <p>Estimados compañeros,</p>
      <p>
        En nuestra comunidad laboral, valoramos la empatía y el apoyo mutuo en
        los momentos de dificultad personal. Sabemos que la vida puede presentar
        desafíos inesperados, y como compañeros, deseamos estar ahí para
        apoyarnos mutuamente en caso de situaciones de emergencia relacionadas
        con la salud de nuestros seres queridos.
      </p>
      <p>
        Por ello, hemos establecido una política de cooperación entre compañeros
        para brindar apoyo financiero a aquellos que se vean afectados por
        hospitalizaciones o fallecimientos de sus familiares más cercanos. Esta
        política se aplica a los siguientes familiares:
      </p>
      <p>
        <ul>
          <li>
            <strong>Hijos</strong>: Incluyendo hijos biológicos, adoptivos y los
            hijos de la pareja.{" "}
          </li>
          <li>
            <strong>Hermanos</strong>: Hermanos y hermanas biológicos y
            adoptivos.
          </li>
          <li>
            <strong>Esposa</strong>: Cónyuge legal o pareja de hecho reconocida.{" "}
          </li>
          <li>
            <strong>Padres</strong>: Padres biológicos y adoptivos.{" "}
          </li>
          <li>
            <strong>Abuelos</strong>: Abuelos y abuelas.{" "}
          </li>
          <li>
            <strong>Suegros</strong>: Padres del cónyuge o pareja de hecho.
          </li>
        </ul>
      </p>
      <p>
        En el caso de que uno de nuestros compañeros operadores o checadores se
        enfrente a una situación de hospitalización o fallecimiento de alguno de
        estos familiares, les alentamos a ponerse en contacto con Abraham
        Cabrera Benito al número 231 159 1893, quien se encargará de comunicar
        la situación a todos los compañeros a través de las redes sociales.
      </p>
      <p>
        Posteriormente, el compañero afectado coordinará la recolección de
        fondos. En caso de que no pueda hacerlo personalmente, deberá designar a
        un familiar o a un amigo entre los compañeros para llevar a cabo esta
        tarea. Alternativamente, otro compañero puede ofrecerse voluntariamente
        para encargarse de la recolección.
      </p>
      <p>
        El apoyo financiero se proporcionará en forma de donaciones voluntarias
        de los compañeros interesados. Cada aportación será completamente
        opcional.
      </p>
      <p>
        Para garantizar la transparencia en el proceso, se requerirá que se
        publique en las redes sociales una lista detallada de cada colaborador y
        el total recaudado. Para ello, se deberá comunicar con Abraham Cabrera
        Benito, quien estará a cargo de la logística y las comunicaciones.
      </p>
      <p>
        Además, se recomienda encarecidamente que al entregar el efectivo, se
        verifique que la cantidad coincida con la registrada en la lista. Esta
        verificación deberá ser confirmada tanto por la persona que entrega como
        por la que recibe.
      </p>
      <p>
        Agradecemos su compromiso con los valores de solidaridad y apoyo en
        nuestro grupo. Juntos, podemos afrontar las dificultades de la vida y
        asegurarnos de que todos nuestros compañeros se sientan respaldados en
        momentos de necesidad.
      </p>
      <p>
        Es importante destacar que estas recaudaciones están destinadas
        exclusivamente para compañeros operadores y checadores.
      </p>
      <p>
        <b>Normativa para la Gestión de Recaudaciones Solidarias.</b>
        <br></br>
        Con el objetivo de mantener la transparencia y la colaboración en las
        recaudaciones destinadas a apoyar a compañeros en situaciones de gastos
        médicos o fallecimiento de un familiar, se establece la siguiente
        dinámica:
      </p>
      <p>
        <b>1. Responsabilidad del beneficiado anterior:</b>
        <br></br>
        El último compañero beneficiado con una recaudación será el responsable
        de llevar a cabo la próxima colecta en caso de que se requiera una
        nueva. Esta medida busca fomentar la reciprocidad y el apoyo mutuo
        dentro del grupo.
      </p>
      <p>
        <b>2. Distribución del tiempo para la recaudación::</b>
        <br></br>
        El responsable deberá dedicar 2 horas en total para realizar la colecta,
        divididas en 2 días consecutivos: <b>Primer día:</b> dedicará 1 hora
        para realizar las solicitudes y organizar la colecta.{" "}
        <b>Segundo día:</b> destinará 1 hora adicional para continuar con la
        gestión y seguimiento.
      </p>
      <p>
        <b>3. Compromiso y apoyo grupal::</b>
        <br></br>
        Este sistema busca distribuir equitativamente las responsabilidades y
        garantizar que todos los compañeros sean apoyados en momentos de
        necesidad. La colaboración de todos es clave para que esta iniciativa
        funcione de manera efectiva.
      </p>

      <div className="separador"></div>
      <h2>Cronología de Recaudaciones.</h2>
      <h1>2021</h1>
      <div className="separador"></div>
      <p>
        01. Recaudación para apoyar al compañero Ivan. Checador de INE.
        Fallecimiento de su Abuelita.jueves, 25 de noviembre de 2021.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./01.png" />
      <h1>2022</h1>
      <div className="separador"></div>
      <p>
        02. Recaudación para apoyar al compañero Candido. Operador de la unidad
        12. Fallecimiento de su señora madre.viernes, 15 de julio de 2022.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./02.png" />
      <div className="separador"></div>
      <p>
        03. Recaudación para apoyar al compañero Aldo. Posturero de la unidad
        43, 38, 18. Fallecimiento de su Padre. Pony unidad 46 U-R.miércoles, 7
        de diciembre de 2022.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./03.png" />
      <div className="separador"></div>
      <h1>2023</h1>
      <p>
        04. Recaudación para apoyar al compañero Fermin. Operador de la unidad
        04. Su hijo se encuentra internado en el hospital.viernes, 20 de enero
        de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./04.png" />
      <div className="separador"></div>
      <p>
        05. Recaudación para apoyar al compañero Ramon. Operador de la unidad
        03. viernes, 24 de febrero de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./05.png" />
      <div className="separador"></div>
      <p>
        06. Recaudación para apoyar al compañero Eloy. Operador de la unidad 25.
        Operación de su señora esposa. martes, 30 de mayo de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./06.png" />
      <div className="separador"></div>
      <p>
        07. Recaudación para apoyar al compañero Jesus. Operador de la unidad
        46. Fallecimiento de su señora esposa. domingo, 18 de junio de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./07.png" />
      <div className="separador"></div>
      <p>
        08. Recaudación para apoyar al compañero Ramiro. Posturero de la unidad
        5, 8, 13, 33. Hospitalizacion de su madre. domingo, 2 de julio de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./08.png" />
      <div className="separador"></div>
      <p>
        09. Recaudación para apoyar al compañero Angel. Operador de la unidad
        43. Se encuentra internado en el hospital.jueves, 17 de agosto de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./09.png" />
      <div className="separador"></div>
      <p>
        10. Recaudación para apoyar al compañero Zamora Posturero de la unidad
        27, 25, 20. Fallecimiento de su Padre.domingo, 15 de octubre de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./10.png" />
      <div className="separador"></div>
      <p>
        11. Recaudación para apoyar al compañero Pedro. Operador de la unidad
        01. Se encuentra hospitalizado en puebla por una bactería en los
        pulmones. lunes, 13 de noviembre de 2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./11.png" />
      <div className="separador"></div>
      <p>
        12. Recaudación para apoyar al compañero Aldo. Posturero de la unidad
        43, 38, 18. Fallecimiento de su abuelito. domingo, 17 de diciembre de
        2023.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./12.png" />
      <div className="separador"></div>
      <h1>2024</h1>
      <div className="separador"></div>
      <p>
        13. Recaudación para apoyar al compañero. Pedro, Operador de la unidad
        01. Se encuentra enfermo. Recauda Heriberto, Eloy y Abraham. jueves, 04
        de enero de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./13.png" />
      <div className="separador"></div>
      <p>
        14. Recaudación para apoyar a la familia del compañero. Pedro, quien en
        vida fue operador de la unidad 01. Fallecimiento. Recauda Heriberto,
        Eloy y Abraham. martes, 26 de marzo de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./14.png" />
      <div className="separador"></div>
      <p>
        15. Recaudación para apoyar al compañero Juan Fernandez. Lavador. Su
        madre se encuentra enferma.Recauda Oscar. sabado, 06 de abril de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./15.png" />
      <div className="separador"></div>
      <p>
        16. Recaudación para apoyar al compañero Heriberto. Operador de la
        unidad 16. Estuvo internado en el hospital y se le realizo una cirugía.
        Recauda Genaro Gato padilla. jueves, 11 de abril de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./16.png" />
      <div className="separador"></div>
      <p>
        17. Recaudación para apoyar al compañero_ David , Operador de la unidad_
        18, (fallecimiento de su suegra). Recauda_ Abraham, Eloy y Heriberto.
        sábado, 20 de abril de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./17.png" />
      <div className="separador"></div>
      <p>
        18. Recaudación para apoyar al compañero_ Samuel, Operador de la unidad_
        10, (Su hija se encuentra internada en el hospital). Recauda_ David,
        Enrique y Abraham. lunes, 6 de mayo de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./18.png" />
      <div className="separador"></div>
      <p>
        19. Recaudación para apoyar al compañero_ Samuel, Operador de la unidad_
        10, (Operaron a su hija de el apéndice). Recaudadores_ Alacrán, Daniel
        #01 y Ramiro. domingo, 12 de mayo de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./19.png" />
      <div className="separador"></div>
      <p>
        20 Recaudación para apoyar al compañero_ Samuel, Operador de la unidad_
        10, (su hija se encuentra internada en el hospital, al día de hoy ya son
        34 días). Recaudan_ Eloy, David y Heriberto. domingo, 9 de junio de 2024
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./20.png" />
      <div className="separador"></div>
      <p>
        21 Recaudación para apoyar al compañero_ Elpidio, Operador de la unidad_
        40, (fallecimiento de su padre). Recauda_ Alacran. martes, 11 de junio
        de 2024
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./21.jpg" />
      <div className="separador"></div>
      <p>
        22 Recaudación para apoyar al compañero_ Sergio, Operador de la unidad_
        02, (su esposa se sometera a una cirugía).Recauda_Samuel y JoyBoy
        domingo, 4 de agosto de 2024
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./22.png" />
      <div className="separador"></div>
      <p>
        23 Recaudación para apoyar al compañero_ Víctor, Checador, (Su esposa
        tuvo un accidente y se encuentra en tratamiento y
        rehabilitación).Recauda_Sergio. Jueves, 12 de septiembre de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./23.png" />
      <div className="separador"></div>
      <p>
        24 Recaudación para apoyar al compañero_ Juan Fernández. Lavador,
        (Fallecimientode su madre). Recauda_Pocoyo. Viernes, 20 de septiembre de
        2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./24.png" />
      <div className="separador"></div>
      <p>
        25 Recaudación para apoyar al compañero_ Samuel, Posturero de la unidad_
        2, 15, 3, 7, (se encuentra grave de los pulmónes). Recauda: Juan
        Fernandez (Lavador). Viernes, 4 de octubre de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./25.png" />
      <div className="separador"></div>
      <p>
        26 Recaudación para apoyar al compañero_ Rodolfo, Operador de la unidad_
        38, (su esposa se encuentra internada en el hospital). Miércoles, 9 de
        octubre de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./26.png" />
      <div className="separador"></div>
      <p>
        27 Recaudación para apoyar al compañero_ Filemon, Operador de la unidad_
        36, (su esposa se opero de emergencia el día Lunes 21 de octubre).lunes,
        28 de octubre de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./27.png" />
      <div className="separador"></div>
      <p>
        28 Recaudación para apoyar al compañero_ Manuel, Operador de la unidad_
        29, (su esposa estuvo internada en el hospital del 02 de noviembre al 05
        de noviembre).jueves, 14 de noviembre de 2024.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./28.png" />
      <div className="separador"></div>
      <h1>2025</h1>
      <div className="separador"></div>
      <p>
        29 Recaudación para apoyar al compañero_ Alejandro, Operador de la
        unidad_ 30, (Su hija se encuentra internada en el hospital general,
        ingreso el dia domingo 05 de enero.).miércoles, 8 de enero de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./29.png" />
      <div className="separador"></div>
      <p>
        30 Recaudación para apoyar al compañero_ Sergio , Operador de la unidad_
        02, (Fallecimiento de su padre el día lunes 6 de enero).lunes, 13 de
        enero de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./30.png" />
      <div className="separador"></div>
      <p>
        31 Recaudación para apoyar al compañero_ Ramiro, Operador de la unidad_
        33, (Operaron a su Madre del apéndice).sábado, 18 de enero de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./31.png" />
      <div className="separador"></div>
      <p>
        32 Recaudación para apoyar al compañero_ Luna , Operador de la unidad_
        39, (Su hijo se encuentra internado en el hospital en puebla). martes, 28
        de enero de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./32.png" />
      <div className="separador"></div>
      <p>
        33 Recaudación para apoyar al compañero_ Javier, Operador de la unidad_
        21, (Estuvo hospitalizado y ahora se encuentra en casa en
        tratamiento). lunes, 10 de febrero de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./33.png" />
      <p>
        34 Recaudación para apoyar al compañero_ Javier, Operador de la unidad_
        21, (Su esposa se encuentra internada en el hospital). sabado, 22 de
        marzo de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./34.jpg" />
      <p>
        35 Recaudación para apoyar al compañero_ Francisco , Operador de la
        unidad_ 16 , (Lo operaron de la vesícula). viernes, 9 de mayo de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./35.png" />
      <p>
        36 Recaudación para apoyar al compañero_ Fermín, Operador de la unidad_
        04, (su hija se encuentra internada en el hospital y se sometera a una
        cirugía de alto riesgo). lunes, 12 de mayo de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./36.png" />
      <p>
        37 Recaudación para apoyar al compañero_ Heriberto, Operador de la
        unidad_ 14, (Tuvo una cirugíaen el pie. Y requiere de mas
        estudios). martes, 27 de mayo de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./37.png" />
      <p>
        38 Recaudación para apoyar al compañero_ Rodolfo, Operador de la unidad_
        38, (Fallecimientode su abuelita). miércoles, 4 de junio de 2025.
      </p>
      <img style={{ width: "95%", height: "auto" }} src="./38.png" />
    </div>
  );
};

export default Recaudaciones;
